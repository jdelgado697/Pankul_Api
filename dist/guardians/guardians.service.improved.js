...
let GuardiansService = class GuardiansService {
    ...
    generateInviteCode(length = 8) {
        return (0, crypto_1.randomBytes)(length).toString("hex").slice(0, length).toUpperCase();
    }
    async list() {
    ...
    async getOne(id) {
        ...
        if (!user) {
            throw new common_1.NotFoundException("Apoderado no encontrado");
        }
        return {
            id: user.id,
            email: user.email,
    ...
    async update(id, body) {
        ...
        if (body.email !== undefined) {
            ...
            if (taken) {
                throw new common_1.BadRequestException("Ese email ya está en uso");
            }
        }
    ...
    async invite(email, playerId) {
        ...
        if (!user) {
            throw new common_1.BadRequestException("Ese email no tiene cuenta. Usa invitación con código.");
        }
        if (user.role !== "PARENT") {
            throw new common_1.BadRequestException("El usuario existe, pero no tiene rol PARENT");
        }
    ...
    async getInvite(code) {
        ...
        if (invite.usedAt) {
            throw new common_1.BadRequestException("La invitación ya fue utilizada.");
        }
        if (invite.expiresAt && invite.expiresAt < new Date()) {
            throw new common_1.BadRequestException("La invitación ha expirado.");
        }
        return {
            ok: true,
            invite: {
    ...
    async acceptInvite(body) {
        ...
        if (!normalizedCode) {
            throw new common_1.BadRequestException("Código requerido.");
        }
        if (!fullName) {
            throw new common_1.BadRequestException("Nombre requerido.");
        }
        ...
        if (existingUser && existingUser.role !== "PARENT") {
            throw new common_1.BadRequestException("Ese correo ya pertenece a otro tipo de usuario.");
        }
    ...
    async updateLink(userId, playerId, data) {
        try {
...
//# sourceMappingURL=guardians.service.improved.js.map