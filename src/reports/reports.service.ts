...
export class ReportsService {
  ...
  async getTeamDashboard() {
    ...
    const evalGroups = new Map<string, typeof evaluations>();
    ...
    for (const [playerId, items] of evalGroups.entries()) {
      ...
      const calcAvg = (x: (typeof evaluations)[number] | undefined) =>
        x
          ? Number(
              (
                (
    ...
    const playersWithMetrics = players.map((player) => {
      ...
      const improvement =
        typeof evalData.latestAverage === "number" &&
    ...
    const teamAttendancePct =
      attendanceValues.length > 0
        ? Math.round(
            attendanceValues.reduce((acc, n) => acc + n, 0) / attendanceValues.length
          )
        : null;
    ...
    const teamAverage =
      averageValues.length > 0
        ? Number(
            (
              averageValues.reduce((acc, n) => acc + n, 0) / averageValues.length
            ).toFixed(2)
          )
        : null;
  ...
  async getPlayerSummary(playerId: string) {
    ...
    const pretty: Record<(typeof fields)[number], string> = {
      dribbling: "Bote",
      passing: "Pase",
      shooting: "Tiro",
      defense: "Defensa",
    ...
    if (typeof latestOverallAverage === "number") {
      if (latestOverallAverage >= 4) overallLabel = "Destacada";
      else if (latestOverallAverage >= 3) overallLabel = "En progreso";
      else overallLabel = "Atención";
    }

    const recommendations: string[] = [];
    if (lastEvaluation) {
      const scored = fields
        .map((f) => ({ f, v: lastEvaluation[f] as number }))
      ...
      focus.forEach((f) => {
        if (f === "dribbling") {
          recommendations.push(
            "Bote: 3 series de 45s (mano fuerte y mano débil), sin mirar el balón."
          );
        } else if (f === "passing") {
          recommendations.push(
            "Pase: 30 pases contra pared (pecho y picado), cuidando precisión."
          );
        } else if (f === "shooting") {
          recommendations.push(
            "Tiro: 25 lanzamientos cortos con buena técnica y equilibrio antes de alejarse."
          );
        } else if (f === "defense") {
          recommendations.push(
            "Defensa: 3 series de 20s de desplazamientos laterales (bajo, manos activas)."
          );
        } else if (f === "decisionMaking") {
          recommendations.push(
            "Toma de decisiones: juegos reducidos 1c1 y 2c1 con lectura rápida y pausa antes de actuar."
          );
        } else if (f === "attitude") {
          recommendations.push(
            "Actitud: reforzar comunicación, esfuerzo constante y respuesta positiva ante correcciones."
          );
        }
      });
    } else {
      recommendations.push(
        "Aún no hay evaluación registrada. El entrenador la agregará pronto."
      );
    }
