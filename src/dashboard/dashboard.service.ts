...
export class DashboardService {
  ...
  async getCoachDashboard() {
    ...
    const attendanceByPlayer = new Map<string, typeof attendances>();
    ...
    const evaluationsByPlayer = new Map<string, typeof evaluations>();
    ...
    const getTrafficLight = (average: number | null, attendancePct: number | null) => {
      if (average === null) return "gray";
      if ((average < 2.5) || (attendancePct !== null && attendancePct < 60)) return "red";
      if ((average < 3.5) || (attendancePct !== null && attendancePct < 75)) return "yellow";
      return "green";
    };
    ...
    const playerStats = players.map((p) => {
      ...
      const attendancePct =
        sessions.length > 0 ? Math.round((attended / sessions.length) * 100) : null;

      const playerEvaluations = (evaluationsByPlayer.get(p.id) || []).sort(
      ...
      const alerts: string[] = [];
      if (attendancePct !== null && attendancePct < 60) alerts.push("Asistencia baja");
      if (latestAverage !== null && latestAverage < 2.5) alerts.push("Desarrollo técnico bajo");
      if (improvement !== null && improvement < 0) alerts.push("Retroceso reciente");
      if (!latest) alerts.push("Sin evaluación");
